...
describe_recipe 'apache2::logrotate' do
  ...
  it 'creates logrotate.d file' do
    case node[:platform]
    when 'debian','ubuntu'
      file("/etc/logrotate.d/apache2").must_exist.with(:mode, '644').and(:owner, 'root').and(:group, 'root')
    when 'centos','redhat','amazon','fedora','scientific','oracle'
      file("/etc/logrotate.d/httpd").must_exist.with(:mode, '644').and(:owner, 'root').and(:group, 'root')
    end
  end
