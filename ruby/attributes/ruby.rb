...
default[:ruby][:version] = "#{node[:ruby][:major_version]}#{node[:ruby][:patch]}"
...
default[:ruby][:deb_url] = "http://download-assets.scalarium.com/#{node[:platform]}/#{node[:platform_version]}/#{node[:ruby][:deb]}"
