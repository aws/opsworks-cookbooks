...
when "2.1"
  ...
  default[:ruby][:patch] = 0
  default[:ruby][:pkgrelease] = '1'
  default[:ruby][:full_version] = "#{node[:ruby][:major_version]}.#{node[:ruby][:patch]}"
when "2.0.0"
  default[:ruby][:major_version] = '2.0'
  default[:ruby][:patch] = 'p353'
  default[:ruby][:full_version] = "#{node["opsworks"]["ruby_version"]}-#{node[:ruby][:patch]}"
  default[:ruby][:pkgrelease] = '1'
when "1.9.3"
...
default[:ruby][:deb] = "opsworks-ruby#{node[:ruby][:major_version]}_#{node[:ruby][:full_version]}.#{node[:ruby][:pkgrelease]}_#{arch}.deb"
...
default[:ruby][:rpm] = "opsworks-ruby#{node[:ruby][:major_version].delete('.')}-#{node[:ruby][:full_version]}-#{node[:ruby][:pkgrelease]}.#{rhel_arch}.rpm"
