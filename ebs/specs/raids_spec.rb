  ...
  describe 'files' do
    it 'creates mdadm.conf' do
      ...
      when 'centos','redhat','amazon'
        file('/etc/mdadm.conf').must_exist.with(:mode, '644').and(:owner, 'root').and(:group, 'root')
      else
        file('/etc/mdadm/mdadm.conf').must_exist.with(:mode, '644').and(:owner, 'root').and(:group, 'root')
      end
    end
  ...
  describe 'mount points' do
    it 'mounts /dev/lvm-raid-[0-9]*/lvm[0-9]* to mount point' do
      node[:ebs][:raids].each do |raid_device, options|
        mount(options[:mount_point], :device => "/dev/lvm-raid-#{raid_device.match(/\d+/)[0]}/lvm#{raid_device.match(/\d+/)[0]}").must_be_mounted.with(:fstype, options[:fstype])
      end
    end
  end
end
