...
if node[:scalarium][:instance][:roles].any?{|role| ['php-app', 'monitoring-master'].include?(role) } || (@roles.include?('rails-app') && node[:scalarium][:rails_stack][:name] == 'apache_passenger')
