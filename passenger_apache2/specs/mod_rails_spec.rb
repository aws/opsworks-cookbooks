...
describe_recipe 'passenger_apache2::mod_rails' do
  ...
  it 'installs mod_passenger', :if => platform?('centos','redhat','amazon') && dist_only? do
    package('mod_passenger').must_be_installed
  end

  it 'removes mod_passenger.conf', :if => platform?('centos','redhat','amazon') && dist_only? do
    file(File.join(node[:apache][:dir], 'conf.d', 'mod_passenger.conf')).wont_exist
  end

  it 'creates passenger.load', :unless => platform?('centos', 'redhat', 'amazon') && dist_only? do
